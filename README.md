...
| `/bandwidth/` | Internal lab - connection bandwidth meter. Full test uses the official `@cloudflare/speedtest` engine (same ramp as speed.cloudflare.com). Live monitor is 6 download streams, periodic upload samples, and a ping every second, so Download, Upload, Latency, and Available now all stay current. Cannot list other LAN devices from the browser; see `~/wifiwatch` for local device discovery. |
| `/spend/` | Internal lab - GitHub Actions minutes, Grok sessions, Claude/Codex dual-review passes. Snapshot from `python3 ~/.grok/scripts/collect-spend.py`. |
